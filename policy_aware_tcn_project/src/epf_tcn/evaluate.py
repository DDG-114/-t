...
from epf_tcn.metrics import daily_metrics, monthly_metrics, regression_summary
...
def evaluate_predictions(predictions: pd.DataFrame, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "daily_accuracy_sum": (
            float(valid_days["accuracy"].sum()) if not valid_days.empty else None
        ),
        "daily_accuracy_avg_by_days": (
            float(valid_days["accuracy"].sum() / valid_days.shape[0])
            if not valid_days.empty
            else None
        ),
