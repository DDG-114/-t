...
def infer_deep_feature_spec(df: pd.DataFrame, config: Dict[str, Any]) -> DeepFeatureSpec:
    ...
    price_context = [
        ...
        if col.startswith("price_lag_") or col.startswith("price_roll_")
    ]
    ...
    hist_cols = _dedupe([target, *exog_cols, *market_cols, *time_cols, *context_cols])
    fut_cols = _dedupe([*exog_cols, *market_cols, *time_cols, *context_cols])
